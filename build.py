...
def check(strict=False):
    ...
    for name, s in SCHEMES.items():
        ...
        floors += [(f"ansi{i}", 4.5) for i in range(1, 7)]
        # ansi0 is the shadow slot and is meant to sit near the ground. Everything
        # else, 7 and 15 included, is a colour programs emit as text.
        floors += [(f"ansi{i}", 3.0) for i in (7, 8, 9, 10, 11, 12, 13, 14, 15)]
        for slot, floor in floors:
            r = contrast(hx(s, slot), bg)
            if r < floor:
                bad.append(f"{name} {slot} {sharp(s, slot)} {r:.2f} < {floor}")
        # Body text and comments must not read as one of the colours they sit
        # next to. Looser than the ansi pairs below, and looser again for the
        # brights: a pale ink near a pale bright is normal in a near-monochrome
        # scheme, a comment the same colour as a string is not.
        for base in ("foreground", "comment"):
            for i, floor in [(j, 10.0) for j in range(1, 7)] + \
                            [(j, 7.5) for j in (9, 10, 11, 12, 13, 14)]:
                d = delta_e(hx(s, base), hx(s, f"ansi{i}"))
                if d < floor:
                    bad.append(f"{name} {base}/ansi{i} dE {d:.1f} < {floor}")
        # Slots carrying different meanings must not read as one colour. Same-hue
        # normal/bright pairs are skipped: looking alike is the point of a bright.
        for i, j in ((1, 3), (1, 5), (2, 3), (2, 6), (4, 6), (6, 12), (1, 13), (2, 14)):
            d = delta_e(hx(s, f"ansi{i}"), hx(s, f"ansi{j}"))
            if d < 12:
                bad.append(f"{name} ansi{i}/ansi{j} dE {d:.1f} < 12")
    for line in bad:
...
PROFILE_BASE = {
    ...
    # PostScript name plus point size, the format iTerm stores fonts in.
    "Normal Font": "IBMPlexMono 12",
...
def build_preview(name, s, path):
    ...
    advance of whichever monospace font the renderer happens to resolve.
    """
    g, a = (lambda k: sharp(s, k)), (lambda i: sharp(s, f"ansi{i}"))
    FS, LH, W, PAD, TOP = 13, 21, 720, 18, 40
    ...
    parts = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" '
             ...
             f'<rect width="{W}" height="30" rx="6" fill="{g("panel")}"/>',
             f'<rect y="24" width="{W}" height="6" fill="{g("panel")}"/>']
    for i, c in enumerate((a(1), a(3), a(2))):
        parts.append(f'<circle cx="{PAD + 5 + i * 15}" cy="15" r="4.5" fill="{c}"/>')
    parts.append(f'<text x="{W - PAD}" y="19" text-anchor="end" font-size="11" '
...
if __name__ == "__main__":
    ...
    build_previews()
    build_palette_json()
    print("built iterm/, vscode/themes/, assets/, palette.json", "(contrast ok)" if ok else "(see warnings)")
