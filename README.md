...
```sh
echo 'source-file "$PWD/tmux/coba.tmux.conf"' >> ~/.tmux.conf
```

Status line, window/pane highlighting and active-pane borders. Colours are
ANSI indices, not hex, so it follows whichever coba profile iTerm has active
and needs no per-variant edit.
...
**Fonts** are not set by the profile: it inherits whatever your `Default`
